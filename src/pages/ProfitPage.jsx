...
const PROFIT_MONTHS = Array.from({ length: 12 }, (_, i) => i + 1);

// ── Utility functions ──
const toNumber = (v) => { const n = Number(v); return Number.isFinite(n) ? n : 0; };
const sumBy = (list, fn) => list.reduce((s, i) => s + fn(i), 0);
const toDateKey = (v) => new Date(v).toISOString().split('T')[0];
const addDays = (d, n) => { const c = new Date(d); c.setDate(c.getDate() + n); return c; };
const isValidDateKey = (v) => typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v);
const fmtMoney = (v) => `${Math.round(toNumber(v)).toLocaleString('ko-KR')}원`;
const fmtSigned = (v) => { const r = Math.round(toNumber(v)); if (!r) return '0원'; return `${r > 0 ? '+' : '-'}${Math.abs(r).toLocaleString('ko-KR')}원`; };
const fmtPct = (v) => `${toNumber(v).toFixed(1)}%`;
const fmtSignedPct = (v) => { const n = toNumber(v); if (Math.abs(n) < 0.0001) return '0.0%'; return `${n > 0 ? '+' : ''}${n.toFixed(1)}%`; };
const getTone = (r) => r < 10 ? 'var(--danger)' : r < 20 ? 'var(--warning)' : 'var(--success)';
...
function buildItemRows(items) {
  const rawRows = items.map(item => {
    ...
    return { name: item.itemName || '(미분류 품목)', code: item.itemCode || '', category: item.category || '미분류', quantity, unitCost, salePrice, hasSalePrice, grossSalesAmount, discountAmount, sgnaExpense, totalCost, totalRevenue, grossProfit, operatingProfit, profit, profitRate, costRatio, operatingProfitRate };
  ...
  rawRows.forEach(row => {
    ...
    if (!groupMap.has(key)) { groupMap.set(key, { ...row }); }
      ...
      const cg = g.grossSalesAmount + row.grossSalesAmount;
      const cd = g.discountAmount + row.discountAmount;
      ...
      g.quantity = cq; g.grossSalesAmount = cg; g.discountAmount = cd; g.sgnaExpense = cs;
      g.totalCost = cc; g.totalRevenue = cr; g.grossProfit = cgp; g.operatingProfit = cop; g.profit = cop;
      g.unitCost = cq > 0 ? Math.round(cc / cq) : 0; g.salePrice = cq > 0 ? Math.round(cg / cq) : 0;
      g.profitRate = cr > 0 ? (cgp / cr) * 100 : 0; g.costRatio = cr > 0 ? (cc / cr) * 100 : 0;
      g.operatingProfitRate = cr > 0 ? (cop / cr) * 100 : 0; g.hasSalePrice = g.hasSalePrice || row.hasSalePrice;
...
function buildPeriodSummary(transactions, items) {
  ...
  transactions.forEach(tx => { const a = getTransactionAmount(tx, items); if (tx.type === 'in') totalIn += a; if (tx.type === 'out') totalOut += a; });
...
function buildMonthlySeries(transactions, items) {
  ...
  transactions.forEach(tx => {
    ...
    if (tx.type === 'in') b.totalIn += a; if (tx.type === 'out') b.totalOut += a;
  });
  return Array.from(map.entries()).sort((a, b) => a[0].localeCompare(b[0])).map(([key, row]) => ({ label: `${key.replace('-', '년 ')}월`, totalIn: Math.round(row.totalIn), totalOut: Math.round(row.totalOut), profit: Math.round(row.totalOut - row.totalIn) }));
}
...
function buildVendorSummary(transactions, items) {
  ...
  transactions.forEach(tx => {
    ...
    cur.count++; if (String(tx.date || '') > cur.lastDate) cur.lastDate = String(tx.date || '');
    const a = getTransactionAmount(tx, items); if (tx.type === 'in') cur.totalIn += a; if (tx.type === 'out') cur.totalOut += a;
    map.set(name, cur);
  });
  return Array.from(map.values()).map(e => { const profit = e.totalOut - e.totalIn; return { ...e, totalIn: Math.round(e.totalIn), totalOut: Math.round(e.totalOut), profit: Math.round(profit), profitRate: e.totalOut > 0 ? (profit / e.totalOut) * 100 : 0, marginRate: e.totalIn > 0 ? (profit / e.totalIn) * 100 : 0 }; });
}

function summarizeByCategory(rows) {
  const map = new Map();
  rows.forEach(row => { const p = map.get(row.category) || { name: row.category, count: 0, revenue: 0, cost: 0, profit: 0 }; p.count++; p.revenue += row.totalRevenue; p.cost += row.totalCost; p.profit += row.profit; map.set(row.category, p); });
  return Array.from(map.values()).map(e => ({ ...e, rate: e.revenue > 0 ? (e.profit / e.revenue) * 100 : 0 })).sort((a, b) => b.profit - a.profit);
}
...
function buildMonthlyPlannerData(transactions, items, year, maps) {
  ...
  (items || []).forEach(item => { const c = String(item.itemCode || '').trim(); const n = String(item.itemName || '').trim(); if (c && !byCode.has(c)) byCode.set(c, item); if (n && !byName.has(n)) byName.set(n, item); });
  const findItem = (tx) => { const c = String(tx.itemCode || '').trim(); if (c && byCode.has(c)) return byCode.get(c); const n = String(tx.itemName || '').trim(); if (n && byName.has(n)) return byName.get(n); return null; };
  const txSummary = {};
  PROFIT_MONTHS.forEach(m => { txSummary[m] = { sales: 0, cost: 0 }; });
  (transactions || []).forEach(tx => {
    if (tx.type !== 'out' || !tx.date) return;
    const m2 = /^(\d{4})-(\d{2})/.exec(String(tx.date));
    if (!m2) return; if (Number(m2[1]) !== year) return;
...
const readLS = (key) => { try { const r = localStorage.getItem(key); return r ? (JSON.parse(r) || {}) : {}; } catch { return {}; } };
const writeLS = (key, obj) => { try { localStorage.setItem(key, JSON.stringify(obj)); } catch {} };
