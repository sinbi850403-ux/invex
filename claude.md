...
### ⚠️ 개발 시 주의
- **main.js는 1,100줄 이상** — 라우팅·사이드바·카드 접기 등이 집중. 분리 필요성 있음
- **style.css는 95KB+** — 디자인 시스템 전체가 한 파일. 컴포넌트별 분리 고려
- **page-inventory.js 84KB** — 가장 큰 페이지. 리팩토링 시 하위 컴포넌트 분리 권장
- **한글 인코딩 깨짐** — 일부 주석이 유니코드 이스케이프로 보임 (기능 영향 없음)
- **프레임워크 없음** — 상태관리·DOM 조작이 수동. 코드량 많지만 의존성 적음
...
| 작업 유형 | 모델 | 예시 |
|-----------|------|------|
| **간단한 수정 / 단순 개발** | Haiku | 버튼 텍스트 변경, import 누락 수정, 스타일 소폭 조정 |
| **중간 난이도 수정 / 기능 개발** | Sonnet | 새 페이지 구현, 로직 리팩토링, API 연동 |
| **심층 설계 / 전체 검사** | Opus | 아키텍처 검토, 보안 감사, 복잡한 알고리즘 설계 |
