-- migrate:up
-- ============================================================
-- Patch: profiles RLS INSERT 권한 부여 + handle_new_user 재정의
-- Source: supabase/fix-profiles-rls.sql
-- ============================================================
...
-- migrate:down
-- 이 마이그레이션은 profiles RLS 정책 + 트리거를 정의함.
-- 후속 패치(20260501120200, 20260501120500)에서 같은 정책이 재정의되므로
-- 단독 롤백은 의미가 없음. 전체 롤백은 dbmate rollback 후 다시 forward.
DO $do$
BEGIN
  RAISE NOTICE '[INVEX] fix_profiles_rls rollback skipped — superseded by later migrations.';
END
$do$;
