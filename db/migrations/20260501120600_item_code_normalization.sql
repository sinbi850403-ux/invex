...
-- ============================================================
-- Patch: items / transactions 코드 기반 정규화 (item_code_norm)
-- Source: supabase/item-code-normalization-2026-04.sql
-- ============================================================
...
-- 정규화 컬럼/인덱스/유니크를 되돌림.
-- transactions.item_id 백필값은 유지 (참조 무결성 보존).
DROP INDEX IF EXISTS idx_tx_user_code_norm;
DROP INDEX IF EXISTS idx_items_user_code_norm;
DROP INDEX IF EXISTS uq_items_user_name_when_no_code;
DROP INDEX IF EXISTS uq_items_user_code_norm;

-- item_name 절대 유니크 복원 (충돌 데이터가 있으면 실패할 수 있음)
DO $do$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'items_user_id_item_name_key'
  ) THEN
    BEGIN
      ALTER TABLE items ADD CONSTRAINT items_user_id_item_name_key UNIQUE (user_id, item_name);
    EXCEPTION WHEN unique_violation THEN
      RAISE NOTICE '[INVEX] items_user_id_item_name_key 복원 실패 — 동명이코드 데이터 존재. 수동 정리 필요.';
    END;
  END IF;
END
$do$;

ALTER TABLE transactions DROP COLUMN IF EXISTS item_code_norm;
ALTER TABLE items        DROP COLUMN IF EXISTS item_code_norm;
