...
-- ============================================================
-- Patch: 2026-04 보안·성능 통합 패치 (CRIT-01 ~ V017)
-- Source: supabase/fix-security-perf-2026-04.sql
--
-- IMPORTANT: transaction:false because CREATE/DROP INDEX CONCURRENTLY
...
-- 이 마이그레이션은 다수의 신규 테이블/컬럼/인덱스/RLS를 추가함.
-- 안전한 단방향 롤백이 어렵고 데이터 손실 위험이 큼.
-- 운영 환경에서는 pg_dump 백업 + 수동 절차 사용을 권장.
DO $do$
BEGIN
  RAISE NOTICE '[INVEX] fix_security_perf rollback intentionally left as no-op. See db/runbook.md.';
END
$do$;
