-- migrate:up
-- ============================================================
-- Patch: team_workspaces 테이블 + RLS + workspace_* RPC + Realtime publication
-- Source: supabase/fix-team-rls.sql
-- ============================================================
...
-- migrate:down
-- team_workspaces 테이블/RPC/RLS 제거.
-- 데이터 손실: team_workspaces 데이터 영구 삭제.
DROP POLICY IF EXISTS "tw_select" ON team_workspaces;
DROP POLICY IF EXISTS "tw_insert" ON team_workspaces;
DROP POLICY IF EXISTS "tw_update" ON team_workspaces;
DROP POLICY IF EXISTS "tw_delete" ON team_workspaces;
...
-- transactions에 추가된 컬럼은 유지 (이후 마이그레이션이 의존).
-- Realtime publication은 후속 변경에서 재구성됨.
DO $do$
BEGIN
  RAISE NOTICE '[INVEX] fix_team_rls rollback: 추가 컬럼/Realtime publication은 보존.';
END
$do$;
